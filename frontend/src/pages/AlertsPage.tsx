...
export default function AlertsPage() {
  ...
  const [sortField, setSortField] = useState('classification')
  ...
  const exportCSV = () => {
    const header = 'Code,Nom,Departement,Classification,Tendance,Derniere mesure,Type\n'
    const rows = alertStations.map(s =>
      `${s.code},"${s.name}",${s.dept},${s.classification},${s.tendance ?? ''},${s.derniere_mesure ?? ''},${s.type}`
    ).join('\n')
