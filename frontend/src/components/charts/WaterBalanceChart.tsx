...
const COMPONENTS = [
  { key: 'wb_effective_precip', label: 'Précipitations eff.', color: '#22c55e' },
...
export function WaterBalanceChart({ data }: Props) {
  ...
  const chartData = useMemo(() => {
    ...
    // Negate evaporation and runoff for display (losses shown below zero)
    return filtered.map(d => ({
      ...d,
      wb_actual_evaporation: d.wb_actual_evaporation != null ? -Math.abs(d.wb_actual_evaporation) : null,
      wb_surface_runoff: d.wb_surface_runoff != null ? -Math.abs(d.wb_surface_runoff) : null,
    }))
  }, [data, period])

  // Check there's any water balance data
  const hasWB = chartData.some(d =>
    d.wb_recharge != null || d.wb_actual_evaporation != null ||
    d.wb_surface_runoff != null || d.wb_effective_precip != null
  )
      ...
      <ResponsiveContainer width="100%" height={260}>
        <AreaChart data={chartData} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
          ...
          <YAxis
            tick={{ fill: '#9ca3af', fontSize: 11 }}
            stroke="transparent"
            label={{ value: 'mm', angle: -90, position: 'insideLeft', fill: '#9ca3af', fontSize: 11 }}
          />
          <Tooltip
            contentStyle={CHART_TOOLTIP_STYLE}
            labelFormatter={(v: any) => new Date(v).toLocaleDateString('fr-FR', { year: 'numeric', month: 'long' })}
            formatter={(value: any, name: any) => {
              const comp = COMPONENTS.find((c: any) => c.key === name)
              return [value != null ? `${Number(value).toFixed(1)} mm` : '—', comp?.label ?? name]
            }}
          />
          {COMPONENTS.map(c => (
            <Area
