...
const ZONE_BANDS: { y1?: number; y2?: number; color: string; label: string }[] = [
  { y2: -1.75, color: 'rgba(153,27,27,0.15)', label: 'Extrêmement bas' },
  { y1: -1.75, y2: -1.28, color: 'rgba(239,68,68,0.12)', label: 'Très bas' },
  { y1: -1.28, y2: -0.84, color: 'rgba(249,115,22,0.12)', label: 'Bas' },
  { y1: -0.84, y2: 0.84, color: 'rgba(34,197,94,0.08)', label: 'Normal' },
  { y1: 0.84, y2: 1.28, color: 'rgba(59,130,246,0.12)', label: 'Haut' },
  { y1: 1.28, y2: 1.75, color: 'rgba(99,102,241,0.12)', label: 'Très haut' },
...
export function DroughtIndexChart({ data, indexKey, label }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(d => new Date(d.mois).getTime() >= cutoffMs)
  }, [data, period])

  // Add fill color per bar
  const chartData = useMemo(() =>
    filtered.map(d => ({
      ...d,
      fill: CLASS_COLORS[d.classification] ?? CLASS_COLORS.UNKNOWN,
    })),
  [filtered])
  ...
  if (!chartData.length) {
    return (
      <div className="flex items-center justify-center h-48 text-text-secondary text-sm">
        Pas assez de données pour calculer l'indice (min. 5 ans)
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <ResponsiveContainer width="100%" height={260}>
        <ComposedChart data={chartData} margin={{ top: 5, right: 5, bottom: 5, left: 5 }}>
          ...
          <YAxis
            ...
            tick={{ fill: '#9ca3af', fontSize: 11 }}
            stroke="transparent"
            label={{ value: indexKey.toUpperCase(), angle: -90, position: 'insideLeft', fill: '#9ca3af', fontSize: 11 }}
          />
          <Tooltip
            contentStyle={CHART_TOOLTIP_STYLE}
            labelFormatter={(v: any) => new Date(v).toLocaleDateString('fr-FR', { year: 'numeric', month: 'long' })}
            formatter={(value: any, name: any) => {
              if (name === indexKey.toUpperCase()) return [value?.toFixed(2) ?? '—', name]
              return [value, name]
            }}
          />
          <Bar
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 justify-end">
        {[
          { label: 'Extr. bas (< -1.75)', color: '#991b1b' },
          { label: 'Très bas', color: '#ef4444' },
          { label: 'Bas', color: '#f97316' },
          { label: 'Normal', color: '#10b981' },
          { label: 'Haut', color: '#3b82f6' },
          { label: 'Très haut', color: '#1d4ed8' },
          { label: 'Extr. haut (> 1.75)', color: '#312e81' },
        ].map(z => (
