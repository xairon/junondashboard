...
/** Fetch + cache a static GeoJSON file */
...
/** WFS feature search config: which fields to search per layer */
const WFS_SEARCH: Record<string, { nameField: string; codeField: string; badge: string; badgeClass: string }> = {
  ...
  'sous-secteur-hydro': { nameField: 'LbSousSecteurHydro',    codeField: 'CdSousSecteurHydro',     badge: 'SS-SECTEUR', badgeClass: 'bg-teal-500/20 text-teal-400' },
...
export function SearchBar({ features, wfsData, onSearchAction }: Props) {
  ...
  // Load static GeoJSON for geographic search
  ...
  const results = useMemo<SearchItem[]>(() => {
    ...
    for (const f of (features ?? [])) {
      ...
      const commune = norm(f.properties.commune ?? '')
      const dept = norm(f.properties.departement ?? '')
      if (code.includes(q) || commune.includes(q) || dept.includes(q)) {
        items.push({
          kind: 'station',
          badge: f.properties.type === 'piezo' ? 'PIEZO' : 'HYDRO',
          badgeClass: f.properties.type === 'piezo' ? 'bg-accent-cyan/20 text-accent-cyan' : 'bg-accent-indigo/20 text-accent-indigo',
          label: f.properties.commune || f.properties.code,
          detail: `${f.properties.departement || ''} · ${f.properties.code}`,
          action: { kind: 'station', code: f.properties.code, label: f.properties.commune || f.properties.code, stationType: f.properties.type, geometry: f.geometry },
        })
        stationCount++
      }
    }
    ...
    for (const f of (deptsGeo?.features ?? [])) {
      ...
      if (code.includes(q) || nom.includes(q)) {
        items.push({
          kind: 'department',
          badge: `${f.properties.code}`,
          badgeClass: 'bg-orange-400/20 text-orange-300',
          label: f.properties.nom,
          detail: `Departement ${f.properties.code}`,
          action: { kind: 'department', code: f.properties.code, label: f.properties.nom, geometry: f.geometry },
        })
        deptCount++
      }
    }
    ...
    for (const f of (regionsGeo?.features ?? [])) {
      ...
      const nom = norm(f.properties.nom)
      if (nom.includes(q) || norm(f.properties.code).includes(q)) {
        items.push({
          kind: 'region',
          badge: 'REGION',
          badgeClass: 'bg-indigo-400/20 text-indigo-300',
          label: f.properties.nom,
          detail: `Region ${f.properties.code}`,
          action: { kind: 'region', code: f.properties.code, label: f.properties.nom, geometry: f.geometry },
        })
        regionCount++
      }
    }
    ...
    for (const f of (bassinsGeo?.features ?? [])) {
      ...
      const code = norm(f.properties.CdBH)
      const nom = norm(f.properties.LbBH)
      if (code.includes(q) || nom.includes(q)) {
        items.push({
          kind: 'bassin',
          badge: f.properties.CdBH,
          badgeClass: 'bg-blue-500/20 text-blue-300',
          label: f.properties.LbBH,
          detail: `Bassin ${f.properties.CdBH}`,
          action: { kind: 'bassin', code: f.properties.CdBH, label: f.properties.LbBH, geometry: f.geometry },
        })
        bassinCount++
      }
    }
    ...
    for (const f of (herGeo?.features ?? [])) {
      ...
      const nom = norm(f.properties.nom)
      const nom1 = norm(f.properties.nom_her1 ?? '')
      if (nom.includes(q) || nom1.includes(q) || String(f.properties.code).includes(q)) {
        items.push({
          kind: 'her',
          badge: 'HER',
          badgeClass: 'bg-emerald-400/20 text-emerald-300',
          label: f.properties.nom,
          detail: f.properties.nom_her1 ? `HER-1: ${f.properties.nom_her1}` : `Code ${f.properties.code}`,
          action: { kind: 'her', code: String(f.properties.code), label: f.properties.nom, geometry: f.geometry },
        })
        herCount++
      }
    }

    // --- WFS layers (cours d'eau, zones hydro, plans d'eau, masses d'eau) ---
    const wfsCounts: Record<string, number> = {}
    if (wfsData) {
      for (const [layerId, config] of Object.entries(WFS_SEARCH)) {
        const data = wfsData[layerId]
        if (!data?.features) continue
        const catKey = config.badge
        if (!wfsCounts[catKey]) wfsCounts[catKey] = 0
        if (wfsCounts[catKey] >= MAX_PER_CAT) continue

        for (const f of data.features) {
          if (wfsCounts[catKey]! >= MAX_PER_CAT) break
          const name = norm(f.properties?.[config.nameField] ?? '')
          const code = norm(f.properties?.[config.codeField] ?? '')
          if (name.includes(q) || code.includes(q)) {
            items.push({
              kind: 'wfs',
              badge: config.badge,
              badgeClass: config.badgeClass,
              label: f.properties?.[config.nameField] ?? f.properties?.[config.codeField] ?? layerId,
              detail: f.properties?.[config.codeField] ?? '',
              action: { kind: 'wfs', code: f.properties?.[config.codeField] ?? '', label: f.properties?.[config.nameField] ?? '', geometry: f.geometry, wfsLayerId: layerId as WfsLayerId },
            })
            wfsCounts[catKey]!++
          }
        }
    ...
    return items.sort((a, b) => {
      const ai = CATEGORY_ORDER.indexOf(a.kind as typeof CATEGORY_ORDER[number])
  ...
  const grouped = useMemo(() => {
    const groups: { kind: string; label: string; items: SearchItem[] }[] = []
    let currentKind = ''
    for (const item of results) {
      if (item.kind !== currentKind) {
        currentKind = item.kind
        groups.push({ kind: item.kind, label: CATEGORY_LABELS[item.kind] ?? item.kind, items: [] })
      }
  ...
  // Flat index across all groups for keyboard navigation
  let flatIndex = -1
    ...
    <div ref={wrapperRef} className="absolute top-4 left-4 z-10 w-72 sm:w-96">
      <div className="relative">
        ...
        <input
          ...
          placeholder="Station, departement, cours d'eau, nappe..."
      ...
      {open && results.length > 0 && (
        <div
          ...
          role="listbox"
          className="mt-1 bg-bg-card/95 backdrop-blur-md border border-white/10 rounded-lg overflow-hidden shadow-xl max-h-80 overflow-y-auto"
        >
          {grouped.map((group) => (
            <div key={group.kind}>
              <div className="px-3 py-1.5 text-[10px] font-medium uppercase tracking-wider text-text-secondary bg-white/[0.03] border-b border-white/5">
                {group.label}
              </div>
              {group.items.map((item) => {
                ...
                return (
                  <button
                    key={`${item.kind}-${item.action.code}-${idx}`}
                    role="option"
                    aria-selected={idx === highlightIndex}
                    onClick={() => selectItem(item)}
                    className={`w-full text-left px-3 py-2 transition-colors border-b border-white/5 last:border-0 ${
                      idx === highlightIndex ? 'bg-bg-hover' : 'hover:bg-bg-hover'
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium shrink-0 ${item.badgeClass}`}>
                        {item.badge}
                      </span>
                      <span className="text-sm text-text-primary truncate">{item.label}</span>
                    </div>
                    {item.detail && (
                      <p className="text-xs text-text-secondary mt-0.5 ml-14 truncate">{item.detail}</p>
                    )}
                  </button>
                )
              })}
      ...
      {open && query.length >= 2 && results.length === 0 && (
        <div className="mt-1 bg-bg-card/95 backdrop-blur-md border border-white/10 rounded-lg px-4 py-3 text-sm text-text-secondary">
